...
import { InjurySystemHandler } from "./injurySystemHandler.js";
...
async function onMessageReceived(mesId) {
	...
	if (extensionSettings.injurySystem?.enabled) {
		...
		const lastTracker = chat[lastMesWithTrackerId]?.tracker ?? {};
		const currentInjuries = lastTracker.PhysicalInjuries || [];
		const psychologicalState = lastTracker.PsychologicalState || {};
		
		const { injuries, psychologicalState: newPsychologicalState } = await InjurySystemHandler.processInjuries(
			chat[mesId].mes,
			currentInjuries,
			psychologicalState
		);
		
		// Update the tracker with new injury information
		if (lastMesWithTrackerId) {
			const updatedTracker = {
				...lastTracker,
				PhysicalInjuries: injuries,
				PsychologicalState: newPsychologicalState
			};
