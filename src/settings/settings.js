...
export const defaultSettings = {
	injurySystem: {
		...
		severityLevels: {
			minor: {
				name: "Minor",
				description: "Small cuts, bruises, or minor sprains",
				recoveryTime: "1-3 days"
			},
			moderate: {
				name: "Moderate",
				description: "Deep cuts, broken bones, or serious sprains",
				recoveryTime: "1-2 weeks"
			},
			severe: {
				name: "Severe",
				description: "Life-threatening injuries, major trauma",
		...
		psychologicalStates: {
			stable: {
				name: "Stable",
				description: "Normal mental state"
			},
			stressed: {
				name: "Stressed",
				description: "Experiencing anxiety or pressure"
			},
			traumatized: {
				name: "Traumatized",
				description: "Severe psychological impact"
			}
		}
	}
};
...
export const extensionSettings = {
	injurySystem: {
		psychologicalStates: {
			stable: {
				name: 'Stable',
				description: 'Character is mentally stable and functioning normally.',
				color: '#4CAF50'
			},
			stressed: {
				name: 'Stressed',
				description: 'Character is experiencing elevated stress levels.',
				color: '#FFC107'
			},
			anxious: {
				name: 'Anxious',
				description: 'Character is showing signs of anxiety and worry.',
				color: '#FF9800'
			},
			traumatized: {
				name: 'Traumatized',
				description: 'Character is experiencing significant psychological trauma.',
				color: '#F44336'
			},
			broken: {
				name: 'Broken',
				description: 'Character is in a severe state of psychological distress.',
				color: '#9C27B0'
			}
		},
		injurySeverities: [
...
export async function initSettings() {
	...
	if (!extensionSettings.injurySystem) {
		extensionSettings.injurySystem = defaultSettings.injurySystem;
	}
...
function setSettingsInitialValues() {
	...
	$("#tracker_injury_system_enable").prop("checked", extensionSettings.injurySystem?.enabled ?? true);
	$("#tracker_injury_system_auto_update").prop("checked", extensionSettings.injurySystem?.autoUpdate ?? true);
	$("#tracker_injury_system_context").val(extensionSettings.injurySystemPrompts?.contextTemplate ?? "");
	$("#tracker_injury_system_prompt").val(extensionSettings.injurySystemPrompts?.systemPrompt ?? "");
	$("#tracker_injury_system_request").val(extensionSettings.injurySystemPrompts?.requestPrompt ?? "");
...
function registerSettingsListeners() {
	...
	// Injury System Settings
	$("#tracker_injury_system_enable").on("input", onSettingCheckboxInput("injurySystem.enabled"));
	$("#tracker_injury_system_auto_update").on("input", onSettingCheckboxInput("injurySystem.autoUpdate"));
	$("#tracker_injury_system_context").on("input", onSettingInputareaInput("injurySystemPrompts.contextTemplate"));
	$("#tracker_injury_system_prompt").on("input", onSettingInputareaInput("injurySystemPrompts.systemPrompt"));
	$("#tracker_injury_system_request").on("input", onSettingInputareaInput("injurySystemPrompts.requestPrompt"));
}
